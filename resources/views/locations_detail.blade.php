...
@section('page')
    ...
    @else
        ...
        @introsection([
            ...
            <p>We'd love to see you this weekend at our {{ $campus->name }} Campus located at {{ $campus->address }}, {{ $campus->city }}, {{ $campus->state }} {{ $campus->zip }}. You'll experience contemporary worship and an engaging message from our Senior Pastor Chris Stephens.</p>
            <p><strong>Service times are:</strong> {!! $campus->times !!}.</p>
        @endintrosection

    @endif

    @profilessection(['id' => 'staff', 'title' => 'Meet the ' . $campus->name . ' Campus Team', 'class' => 'Section--lightGrey', 'profiles' => $campus->Staff])
    @endprofilessection

    @cardsection(['title' => 'What to Expect', 'cards' => $what_to_expect_cards])
